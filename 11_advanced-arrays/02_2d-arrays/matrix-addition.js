...
function matrixAddition(arr1, arr2) {
    ...
    for (let i = 0; i < arr1.length || i < arr2.length; i++) {
        let subArray1 = arr1[i];
        let subArray2 = arr2[i];

        let newSubArray = [];
        for (let j = 0; j < subArray1.length || j < subArray2.length; j++) {
            let num1 = subArray1[j];
            let num2 = subArray2[j];
            newSubArray.push(num1 + num2);
        }
        maxtrixSum.push(newSubArray);
    }
